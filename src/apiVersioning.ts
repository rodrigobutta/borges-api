...
export default (config: any, callback: any) => {
  ...
  const { test, entryPoint, apiPath, instance } = mergedConfig;

  console.log('app use versioning', test, entryPoint, apiPath);

  return (httpRequest: any, httpResponse: any, next: any) => {
    if (!apiPath) {
    ...
    if (version) {
      ...
      return callback(
        ...
        httpResponse,
        next,
      ); // we can't find the entry point, throw an error
    }
